		...
		onTheDetailPage: {
			...
			assertions: {
				...
				iShouldSeeTheRememberedObjectId: function() {
					return this.waitFor({
						success: function() {
							var sBindingPath = "/RequestSet('" + this.getContext().currentId + "')";
							this._waitForPageBindingPath(sBindingPath);
						}
					});
				},
					...
					return this.waitFor(this.createAWaitForAnEntitySet({
						...
						success: function(aEntitySet) {
							...
							return this.waitFor({
								...
								success: function(oList) {
									var sObjectID = oList.getBindingContext().getProperty("AppId");

									var iLength = aEntitySet.filter(function(oLineItem) {
										return oLineItem.AppId === sObjectID;
									}).length;
				...
				theShareTileButtonShouldContainTheRememberedObjectName: function() {
					return this.waitFor({
						...
						matchers: function(oButton) {
							var sObjectName = this.getContext().currentListItem.getBindingContext().getProperty("Field01");
